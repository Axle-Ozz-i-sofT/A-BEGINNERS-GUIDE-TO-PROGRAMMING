...
# distinctly different API to the default C API which goes against my
# primary goal of exemplifying the same code routines in all 3 languages (C,
# FreeBASIC and Python 3).
...
#
#
# data in SQLite 3 is stored internally as bytes by default. The length of the
# bytes and the affinity (data type) is stored in the header entry of the data.
...
def db_list_table_rows_data(db_file_name, db_table_name, db_tbl_rowdata, number_columns):
    ...
    # char *p_db_table1 = "DROP TABLE IF EXISTS Hrs_worked_Tracker;";
    #"create table aTable(field1 int); drop table aTable;
    # We can only send one query at a time to sqlite3.
