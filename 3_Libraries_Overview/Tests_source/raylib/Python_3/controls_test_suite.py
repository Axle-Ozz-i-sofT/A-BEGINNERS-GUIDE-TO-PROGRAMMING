...
## Convert list to 2D C char *Array[] (See: Line 71)
...
#SetExitKey(0)  # No key pressed # NameError: name 'SetExitKey' is not defined
set_exit_key(0)
...
## Use the following 3 lines or ...
#c_array_2d = [ffi.new("char[]", x.encode('utf-8')) for x in list1]
#list_list_view_ex_list = ffi.new("char *[]", c_array_2d)
# keep 'items' alive as long as you need 'list_list_view_ex_list'
list_list_view_ex_list = list_to_2d_c_array(list1)  ## Use this line with the function.


str_multi_str_text_box_text = "Multi text box"
...
struct_view_scroll = Vector2( 0, 0 )  ## class pyray.Vector2(x, y) (C struct)

# Moved from Main game loop
struct_dropped_files = FilePathList  # class pyray.FilePathList(capacity, count, paths) (C struct)

struct_view = Rectangle(0, 0, 0, 0)  ## class pyray.Rectangle(x, y, width, height) (C struct)
#----------------------------------------------------------------------------------
...
str_text_input = ""  # C string array
bool_show_text_input_box = False

str_text_input_file_name = ""  # C string array
...
while (False == bool_exit_window):    # Detect window close button or ESC key
    ...
    if (gui_spinner(Rectangle( 25, 135, 125, 30 ), None, int_spinner_001_value, 0, 100, bool_spinner_edit_mode)):
        bool_spinner_edit_mode = not bool_spinner_edit_mode
    if (gui_value_box(Rectangle( 25, 175, 125, 30 ), None, int_value_box_002_value, 0, 100, bool_value_box_edit_mode)):
        bool_value_box_edit_mode = not bool_value_box_edit_mode

    gui_set_style(TEXTBOX, TEXT_ALIGNMENT, TEXT_ALIGN_LEFT)

    if (gui_text_box(Rectangle( 25, 215, 125, 30 ), str_text_box_text, 64, bool_text_box_edit_mode)):
    ...
    int_list_struct_view_ex_active = gui_list_view_ex(Rectangle( 165, 180, 140, 200 ), list_list_view_ex_list, 8, int_list_view_ex_focus, int_list_view_ex_scroll_index, int_list_view_ex_active)

    int_toggle_group_active = gui_toggle_group(Rectangle( 165, 400, 140, 25 ), "#1#ONE\n#3#TWO\n#8#THREE\n#23#", int_toggle_group_active)

    # Third GUI column
    if (gui_text_box_multi(Rectangle( 320, 25, 225, 140 ), str_multi_str_text_box_text, 256, bool_multi_bool_text_box_edit_mode)):
    ...
    if (bool_show_message_box):
        ...
        result = GuiMessageBox(Rectangle( float(get_screen_width())/2 - 125, float(get_screen_height())/2 - 50, 250, 100 ), gui_icon_text(ICON_EXIT, "Close Window"), "Do you really want to exit?", "Yes;No")
    ...
    if (bool_show_text_input_box):
        draw_rectangle(0, 0, get_screen_width(), get_screen_height(), fade(RAYWHITE, 0.8));
        result = gui_text_input_box(Rectangle( float(get_screen_width())/2 - 120, float(get_screen_height())/2 - 60, 240, 140 ), "Save", gui_icon_text(ICON_FILE_SAVE, "Save file as..."), "Ok;Cancel", str_text_input, 255, None)

        if (result == 1):
            # TODO: Validate str_text_input value and save

            str_text_input_file_name = str_text_input  # strcpy(,)

        if ((result == 0) or (result == 1) or (result == 2)):
            bool_show_text_input_box = False
            str_text_input = ""  # strcpy(str_text_input, "\0")
